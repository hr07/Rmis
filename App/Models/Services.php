...
class Services
{
    ...
    public function  GetAdminProfile(string $username, array $columns, int $fetchMode = 0){
        $this->conn->runSQL("SELECT admin_id FROM administrators WHERE username = ?", array($username), 1);
        if($this->conn->getCount()){
            $adminId = $this->conn->getOutput()->admin_id;
            $this->conn->setTable("administrators");
            $this->conn->setField($columns);
            $this->conn->setQueryIdentifier(array("admin_id" => $adminId));
            $this->conn->queryDb("select", $fetchMode);
            $this->results = $this->conn->getOutput();
            $this->count = $this->conn->getCount();
            return true;
        }
